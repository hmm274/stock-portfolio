...
const StockDetails = () => {
  ...
  const checkIfStockIsAdded = useCallback(async (userId) => {
    ...
    if (!error && data.length > 0) setStockAdded(true);
  }, [symbol]);
  ...
  const timeSeries = stockData['Time Series (Daily)'] || stockData['Weekly Time Series'];
    ...
    <div>
      ...
      <div className="StockDetails">
        ...
        <div className="stock-info">
          ...
          <p><strong>Profit Margin:</strong> {priceDifference < 0 ? `-$${(-priceDifference).toFixed(2)}` : `$${priceDifference.toFixed(2)}`}</p>
        </div>
        <hr />
        <div>
          <Line data={chartData} style={{ minWidth: '500px', minHeight:'350px', maxWidth:'100%', margin: 'auto' }} />
        </div>
        <hr />
